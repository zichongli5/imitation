...
from make_envs import make_env, create_env
from dataset.memory import Memory
from agent import make_agent
from utils_iq.utils import eval_mode, average_dicts, get_concat_samples, evaluate, soft_update, hard_update
from utils_iq.logger import Logger
...
def main(cfg: DictConfig):
    ...
    stats_path = f'/home/zli911/imitation/expert_files/rl-trained-agents/ppo/{args.env.name}_1/{args.env.name}'
    hyperparams, _ = get_saved_hyperparams(stats_path)
    print(hyperparams)
    env_wrapper = get_wrapper_class(hyperparams)
    stats_path = stats_path + '/vecnormalize.pkl'
    env = create_env(args.env.name, n_envs=1, norm_obs=True, norm_reward=False, stats_path=stats_path, seed=0, env_wrapper=env_wrapper, manual_load=True)
    eval_env = create_env(args.env.name, n_envs=1, norm_obs=True, norm_reward=False, stats_path=stats_path, seed=0, env_wrapper=env_wrapper, manual_load=True)
    
    trajectory_path = f'/home/zli911/imitation/expert_files/{args.env.name}/ppo/IQ_trajs_norm.pkl'
    # env = make_env(args)
    ...
    expert_memory_replay.load(trajectory_path,
    ...
    for epoch in count():
        ...
        for episode_step in range(EPISODE_STEPS):
            ...
            if learn_steps % args.env.eval_interval == 0:
                ...
                if returns > best_eval_returns:
                    ...
                    save(agent, epoch, args, output_dir='/home/zli911/imitation/baselines/IQ-Learn/iq_learn/results_best')
        ...
        save(agent, epoch, args, output_dir='/home/zli911/imitation/baselines/IQ-Learn/iq_learn/results')


def save(agent, epoch, args, output_dir='/home/zli911/imitation/baselines/IQ-Learn/iq_learn/results'):
    if epoch % args.save_interval == 0:
        ...
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        agent.save(f'{output_dir}/{args.agent.name}_{name}_{args.expert.demos}')
