...
class Memory(object):
    ...
    def get_samples(self, batch_size, device):
        ...
        batch_reward = torch.as_tensor(np.array(batch_reward), dtype=torch.float, device=device).unsqueeze(1)
        batch_done = torch.as_tensor(np.array(batch_done), dtype=torch.float, device=device).unsqueeze(1)
